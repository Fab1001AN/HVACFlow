...
export default function EngineeringDashboard() {
 ...
 return <div><PageHeader title="Engineering Manager" description="Stages are locked in order and cannot be skipped." />{isLoading?<div className="p-12 flex justify-center"><Spinner/></div>:<div className="p-6 space-y-4">{data.map((u:any)=>{const i=steps.indexOf(u.engineeringStatus);const next=steps[i+1];return <Card key={u.id} className="p-4"><div className="flex flex-col lg:flex-row lg:items-center gap-4"><div className="lg:w-56"><Link href={`/units/${u.id}`} className="font-semibold hover:text-primary">{u.serialNumber}</Link><div className="text-xs text-muted-foreground">{u.unitType?.name}</div></div><div className="flex-1 flex gap-1 overflow-x-auto">{steps.map((s,idx)=><div key={s} className={`min-w-28 rounded px-2 py-2 text-center text-[11px] border ${idx<i?'bg-primary/10 border-primary/30':idx===i?'bg-card border-primary':'opacity-45'}`}>{labels[s]}</div>)}</div><div className="lg:w-52">{next?<Button className="w-full" size="sm" loading={advance.isPending&&advance.variables===u.id} onClick={()=>advance.mutate(u.id)}>Mark {labels[next]}</Button>:<Badge variant="outline">Engineering Complete</Badge>}</div></div></Card>})}</div>}</div>
}
